...
subroutine write_profile(unit,profile)
  ...
  integer :: levels, secs, days, i, j, k, nlinks
  real :: days_since, station
  real, dimension(MAX_LEVELS_FILE) :: flag
  logical, dimension(MAX_LEVELS_FILE) :: lflag
  integer :: findex
  logical :: debug=.false.
  ...
  lflag(1:levels)=profile%flag(1:levels)
  do k=1,levels
    if (lflag(k)) then
       flag(k)=1.0
    else
  ...
  do i = 1, nlinks
     ...
     lflag(1:levels)=profile%flag((MAX_LEVELS_FILE*i)+1:(MAX_LEVELS_FILE*i)+levels)
     do k=1,levels
       if (lflag(k)) then
          flag(k)=1.0
       else
